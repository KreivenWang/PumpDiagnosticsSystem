...
| Order | Logic           | Redis                          |
| ----- | --------------- | ------------------------------ |
| 1     | 判断机泵在运行         | {PPGUID}_Power==1              |
| 2     | 判断转速传感器 存在数据更新  | {SSGUID}_SPEED_PICKDATE != 上一次 |
| 2     | 判断振动传感器 存在数据更新  | {SSGUID}_PICKDATE != 上一次       |
| 2     | 判断非振动传感器 存在数据更新 | IntouchUpdateTime != 上一次       |
...
|   Id    | PSGuid  | SampleInv | GradeCount | LaunchTime |   RestartTime   |  RestartCount   |
| :-----: | :-----: | :-------: | :--------: | :--------: | :-------------: | :-------------: |
| 运行记录的Id | 泵站的Guid |   采样频率    |   故障分档情况   |   软件启动时间   | 软件重启时间 （实时诊断模式） | 重启次数   （实时诊断模式） |

## 历史诊断模式

即历史数据的诊断模式。在该模式下，软件每一次启动后，都将生成新的运行记录，所有的运行和诊断数据都基于该次运行记录。

## 实时诊断模式

即历史数据的诊断模式。在该模式下，软件每一次启动后，都将更新最后一条运行记录，所有的运行和诊断数据都基于该次运行记录。

### 注意点

当历史模式切换为实时模式时，会紧接最后一次运行记录，设置为重启，而不是新记录，从而导致不同时间段的数据冲突。

所以在遇到此问题时，先把数据库备份清空，再次运行。